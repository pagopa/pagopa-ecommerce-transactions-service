...
| NODE_FORWARDER_API_KEY                          |     | Checkout Redirect forwarder api key                                                                                                                                | string  |         |
| REDIRECT_PSP_LIST                               |     | List of all psp ids that are expected to be found into the REDIRECT_URL_MAPPING and REDIRECT_LOGO_MAPPING  configuration (used for configuration cross validation) | string  |         |
| NODE_FORWARDER_URL                              |     | Checkout Redirect forwarder backend URL                                                                                                                            | string  |         |
| REDIRECT_URL_MAPPING                            |     | Key-value string map PSP to backend URI mapping that will be used for Checkout Redirect payments                                                                   | string  |         |
| NODE_FORWARDER_READ_TIMEOUT                     |     | Checkout Redirection request read timeout                                                                                                                          | number  |         |
| NODE_FORWARDER_CONNECTION_TIMEOUT               |     | Checkout Redirection request connection timeout                                                                                                                    | number  |         |
| REDIRECT_LOGO_MAPPING                           |     | Checkout Redirection PSP logo mappings. Those logos will be used in user notification email                                                                        | string  |         |
