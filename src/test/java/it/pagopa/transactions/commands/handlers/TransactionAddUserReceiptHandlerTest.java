...
class TransactionAddUserReceiptHandlerTest {
    ...
    @Test
    void shouldSendNotificationErrorEventForSendPaymentResultOKWithErrorCallingNotificationsService() {
        TransactionActivatedEvent transactionActivatedEvent = transactionActivateEvent();
        ...
        TransactionUserReceiptAddErrorEvent event = transactionUserReceiptAddErrorEvent(
                TransactionUserReceiptData.Outcome.OK
        );
        ...
        Mockito.when(userReceiptDataEventRepository.save(any())).thenAnswer(a -> Mono.just(a.getArgument(0)));
        ...
        StepVerifier.create(updateStatusHandler.handle(transactionAddUserReceiptCommand))
                .expectNextMatches(next -> {
                    ...
                    /*
                     * workaround: saved event is created and creation date and id differs from the
                     * mocked one since those fields are autogenerated by event constructor
                     */
                    assertNotNull(errorEvent);
                    errorEvent.setId(event.getId());
                    errorEvent.setCreationDate(event.getCreationDate());
        ...
        Mockito.verify(userReceiptDataEventRepository, Mockito.times(1)).save(
                argThat(
                        eventArg -> TransactionEventCode.TRANSACTION_ADD_USER_RECEIPT_ERROR_EVENT
                                .equals(eventArg.getEventCode())
                )
        );
        Mockito.verify(refundedRequestedEventStoreRepository, Mockito.times(0)).save(any());
        Mockito.verify(transactionRefundQueueClient, Mockito.times(0))
                .sendMessageWithResponse(any(BinaryData.class), any(), any());
        NotificationsServiceClient.SuccessTemplateRequest successTemplateRequest = successTemplateMailCaptor
                .getAllValues().get(0);
        assertEquals(EMAIL_STRING, successTemplateRequest.to());
    }
    ...
    @Test
    void shouldSendNotificationErrorEventForSendPaymentResultKOWithErrorCallingNotificationsService() {
        TransactionActivatedEvent transactionActivatedEvent = transactionActivateEvent();
