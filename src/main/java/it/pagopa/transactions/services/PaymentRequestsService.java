...
public class PaymentRequestsService {
    ...
    public Mono<PaymentRequestsGetResponseDto> getPaymentRequestInfo(String rptId) {
        ...
        return getPaymentInfoFromCache(rptIdRecord)
                .doOnNext(
                        paymentRequestFromCache -> log.info(
                ...
                .switchIfEmpty(
                        Mono.defer(
                                () -> getPaymentInfoFromNodo(rptIdRecord, paymentContextCode)
                                        .doOnNext(
                                                paymentRequestFromNodo -> log.info(
                                        ...
                                        .doOnSuccess(
                                                paymenRequestInfo -> paymentRequestsInfoRepository
                                                        .save(paymenRequestInfo)
                                        )
                        )
                )
                .map(
                        paymentInfo -> new PaymentRequestsGetResponseDto()
                ...
                .doOnNext(
                        paymentInfo -> log.info("PaymentRequestInfo retrived for {}: {}", rptId, paymentInfo != null)
                );
    }

    private Mono<PaymentRequestInfo> getPaymentInfoFromCache(RptId rptId) {

        Optional<PaymentRequestInfo> paymentRequestInfoOptional = paymentRequestsInfoRepository.findById(rptId);
        return paymentRequestInfoOptional.map(Mono::just).orElseGet(Mono::empty);
    }

    private Mono<PaymentRequestInfo> getPaymentInfoFromNodo(
                                                            RptId rptId,
        ...
        return Mono.just(rptId)
                ...
                .flatMap(
                        nodoVerificaRPTResponse -> {
                            final EsitoNodoVerificaRPTRisposta nodoVerificaRPTRResponse = nodoVerificaRPTResponse
                                    .getNodoVerificaRPTRisposta();
                ...
                .flatMap(
                        args -> {
                            ...
                            if (Boolean.TRUE.equals(isNM3)) {
                                ...
                                paymentRequestInfo = nodeForPspClient
                                        .verifyPaymentNotice(
                                        ...
                                        .map(
                                                verifyPaymentNoticeRes -> new PaymentRequestInfo(
                                                        rptId,
                                ...
                                final CtEnteBeneficiario enteBeneficiario = nodoVerificaRPTRResponse
                                        .getDatiPagamentoPA().getEnteBeneficiario();
                                ...
                                paymentRequestInfo = Mono.just(
                                        new PaymentRequestInfo(
                                                ...
                                                enteBeneficiario != null
                                                        && enteBeneficiario
                                                                .getIdentificativoUnivocoBeneficiario() != null
                                                                        ? enteBeneficiario
