...
public class LogoMappingUtils {
    ...
    @Autowired
    public LogoMappingUtils(Map<CardAuthRequestDetailsDto.BrandEnum, URI> pgsBrandConfMap, Map<String, URI> npgPaymentCircuitLogoMap) {
        this.pgsBrandConfMap = pgsBrandConfMap;
        this.npgPaymentCircuitLogoMap = npgPaymentCircuitLogoMap;
    }


    public URI getLogo(RequestAuthorizationRequestDetailsDto authorizationRequestDetailsDto) {
        return switch (authorizationRequestDetailsDto) {
            case CardAuthRequestDetailsDto details -> pgsBrandConfMap.get(details.getBrand());
            case CardsAuthRequestDetailsDto details -> {
                URI unknown = npgPaymentCircuitLogoMap.get(BrandLogoConfig.UNKNOWN_LOGO_KEY);
                yield npgPaymentCircuitLogoMap.getOrDefault(details.getDetailType(), unknown);
            }
            default -> null;
        };
    }
