...
public class MDCCachingValuesServerHttpRequestDecorator extends ServerHttpRequestDecorator {
    ...
    /**
     * Cache body values and search for json object with listed properties. Enrich
     * this list of properties as needed to cache more value as possible if needed
     */
    @SneakyThrows
    ...
    public String getInfoFromValuesMap() {
        ...
        objectAsMap.keySet().stream().filter(k -> k.equals(RPT_ID) || k.equals(PAYMENT_CONTEXT_CODE)).forEach(
                filteredKey -> builder.append(String.format("[%s]: [%s] ", filteredKey, objectAsMap.get(filteredKey)))
        );
