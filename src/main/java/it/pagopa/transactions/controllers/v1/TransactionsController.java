    ...
    public Mono<ResponseEntity<RequestAuthorizationResponseDto>> requestTransactionAuthorization(
        ...
        return requestAuthorizationRequestDto
                ...
                .map(updateAuthorizationRequest -> {
                    ...
                    // fix CHK-3222: auth request operations are not idempotent NPG side, so this
                    // lock prevents multiple auth request to be performed for a single transaction
                    // (for timeouts scenario and so on). the lock duration have been set to the
                    // payment token validity time in order to make this api call performable only
                    // once for transaction (further attempts will found transaction in expired
